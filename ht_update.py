...
from base_update import BaseConfig, BaseRCA, RCAResults
...
class HT(BaseRCA):
    ...
    def train(self, normal_df: pd.DataFrame, **kwargs):
        ...
        For each node in the graph, if it has parent nodes then a model (specified by model_type)
        ...
        for node in list(self.graph):
            ...
            if parents:
                ...
                if normal_x.shape[1] > 0:
                    # Create the model based on the selected model_type.
    ...
    def find_root_causes(
        ...
        **kwargs,
        ...
        the shortest paths from each candidate root to the anomaly are included.
        """
        node_scores = {}
        for node in list(self.graph):
            parents = list(self.graph.predecessors(node))
